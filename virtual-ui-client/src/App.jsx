...
function App() {
  ...
  useEffect(() => {
    ...
    const fetchExtraData = async () => {
      // allSettled: /all-users is admin-only, so a 403 for normal users
      // must not stop the components list from loading.
      const [usersRes, componentsRes] = await Promise.allSettled([
      ...
      if (usersRes.status === "fulfilled") {
        dispatch(setAllUsers(usersRes.value.data));
      }

      if (componentsRes.status === "fulfilled") {
        dispatch(setAllComponents(componentsRes.value.data));
      }
    };
